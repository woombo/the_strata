 ...
 */
class TheStrataThemeContentFormHelper implements ContainerInjectionInterface {

  use AjaxHelperTrait;
  use StringTranslationTrait;

  /**
   * TheStrataThemeContentFormHelper constructor.
   *
  ...
  public function formAlter(array &$form, FormStateInterface $form_state, $form_id) {
    ...
    if (($use_sticky_action_buttons || $is_content_form) && isset($form['actions'])) {
      ...
      $form['the_strata_theme_sticky_actions'] = [
        '#type' => 'container',
        '#weight' => -1,
        '#multilingual' => TRUE,
        '#attributes' => [
      ...
      // Create the_strata_theme_more_actions group.
      $toggle_more_actions = $this->t('More actions');
      $form['the_strata_theme_sticky_actions']['more_actions'] = [
        '#type' => 'container',
        '#multilingual' => TRUE,
        '#weight' => 998,
        '#attributes' => [
        ...
        'more_actions_toggle' => [
          '#markup' => '<a href="#toggle-more-actions" class="gin-more-actions__trigger trigger" data-gin-tooltip role="button" title="' . $toggle_more_actions . '" aria-controls="the_strata_theme_more_actions"><span class="visually-hidden">' . $toggle_more_actions . '</span></a>',
          '#weight' => 1,
        ],
        'more_actions_items' => [
          '#type' => 'container',
      ...
      // Assign status to the_strata_theme_actions.
      $form['the_strata_theme_sticky_actions']['status'] = [
        '#type' => 'container',
        '#weight' => -1,
        '#multilingual' => TRUE,
      ];
      ...
      $form['the_strata_theme_move_focus_to_sticky_bar'] = [
    ...
    if (isset($form['actions'])) {
      ...
      $form['the_strata_theme_sticky_actions']['the_strata_theme_sidebar_toggle'] = [
        '#markup' => '<a href="#toggle-sidebar" class="meta-sidebar__trigger trigger" data-gin-tooltip role="button" title="' . $hide_panel . '" aria-controls="the_strata_theme_sidebar"><span class="visually-hidden">' . $hide_panel . '</span></a>',
        '#weight' => 1000,
      ];
      $form['#attached']['library'][] = 'gin/sidebar';

      // Create the_strata_theme_sidebar group.
      $form['the_strata_theme_sidebar'] = [
        '#group' => 'meta',
        '#type' => 'container',
        '#weight' => 99,
        '#multilingual' => TRUE,
      ...
      // Copy footer over.
      $form['the_strata_theme_sidebar']['footer'] = ($form['footer']) ?? [];

      // Sidebar close button.
      $close_sidebar_translation = $this->t('Close sidebar panel');
      $form['the_strata_theme_sidebar']['the_strata_theme_sidebar_close'] = [
      ...
      $form['the_strata_theme_sidebar_overlay'] = [
  ...
  public static function formAfterBuild(array $form, FormStateInterface $form_state): array {
    ...
    $form['the_strata_theme_sticky_actions']['more_actions']['more_actions_items']['#weight'] = 2;
    $form['the_strata_theme_sticky_actions']['more_actions']['more_actions_items']['#attributes']['class'] = ['gin-more-actions__menu'];
    ...
    foreach (Element::children($form['actions']) as $key) {
      ...
      if (_the_strata_theme_module_is_active('navigation')) {
        $form['the_strata_theme_sticky_actions']['actions'][$key] = $button;
      }
      ...
      if ($button['#type'] ?? '' === 'submit' || $form['#form_id'] === 'media_type_add_form') {
        ...
        if (_the_strata_theme_module_is_active('navigation') || in_array($key, $includes, TRUE) || !empty($button['#the_strata_theme_action_item'])) {
          $form['the_strata_theme_sticky_actions']['actions'][$key] = $button;
        }
        // Add to more menu.
        else {
          $form['the_strata_theme_sticky_actions']['more_actions']['more_actions_items'][$key] = $button;
        }
      }
      // Else add button to more menu.
      elseif (!in_array($key, $includes, TRUE)) {
        $form['the_strata_theme_sticky_actions']['more_actions']['more_actions_items'][$key] = $button;
        $form['the_strata_theme_sticky_actions']['more_actions']['more_actions_items'][$key]['#attributes']['form'] = $button['#id'];
      }
    }

    if (_the_strata_theme_module_is_active('navigation')) {
      unset($form['the_strata_theme_sticky_actions']['more_actions']);
    }

    _the_strata_theme_form_actions($form['the_strata_theme_sticky_actions'] ?? NULL);
    unset($form['the_strata_theme_sticky_actions']);
  ...
  private function stickyActionButtons(?array $form = NULL, ?FormStateInterface $form_state = NULL, $form_id = NULL): bool {
    ...
    $settings = $this->classResolver->getInstanceFromDefinition(TheStrataThemeSettings::class);
    ...
    // API check.
    $form_ids = $this->moduleHandler->invokeAll('the_strata_theme_ignore_sticky_form_actions');
    $this->moduleHandler->alter('the_strata_theme_ignore_sticky_form_actions', $form_ids);
    $this->themeManager->alter('the_strata_theme_ignore_sticky_form_actions', $form_ids);
   ...
   *
   * _the_strata_theme_is_content_form() is replaced by
   * \Drupal::classResolver(TheStrataThemeContentFormHelper::class)->isContentForm().
   *
   * @param array $form
   *   An associative array containing the structure of the form.
   * @param \Drupal\Core\Form\FormStateInterface $form_state
  ...
  public function isContentForm(?array $form = NULL, ?FormStateInterface $form_state = NULL, $form_id = ''): bool {
    ...
    // the_strata_theme_preprocess_html is not triggered here, so checking
    ...
    // API check.
    $additional_routes = $this->moduleHandler->invokeAll('the_strata_theme_content_form_routes');
    $route_names = array_merge($additional_routes, $route_names);
    $this->moduleHandler->alter('the_strata_theme_content_form_routes', $route_names);
    $this->themeManager->alter('the_strata_theme_content_form_routes', $route_names);

    if (
      in_array($route_name, $route_names, TRUE) ||
      ($form_state && ($form_state->getBuildInfo()['base_form_id'] ?? NULL) === 'node_form') ||
      ($route_name === 'entity.group_content.create_form' && substr($this->routeMatch->getParameter('pluthe_strata_theme_id'), 0, 11) === "group_node:") ||
      ($route_name === 'entity.group_relationship.create_form' && substr($this->routeMatch->getParameter('pluthe_strata_theme_id'), 0, 11) === "group_node:")
    ) {
