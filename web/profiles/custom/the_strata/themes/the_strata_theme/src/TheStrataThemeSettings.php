 ...
 */
class TheStrataThemeSettings implements ContainerInjectionInterface {
  ...
  public function get($name, ?AccountInterface $account = NULL) {
    ...
    if ($this->userOverrideEnabled($account)) {
      $settings = $this->userData->get('the_strata_theme', $account->id(), 'settings');
      if (isset($settings[$name])) {
        $value = $settings[$name];
      }
      else {
        // Try loading legacy settings from user data.
        $value = $this->userData->get('the_strata_theme', $account->id(), $name);
      }
    }
    if (is_null($value)) {
  ...
  public function setAll(array $settings, ?AccountInterface $account = NULL) {
    ...
    $this->userData->delete('the_strata_theme', $account->id());
    $this->userData->set('the_strata_theme', $account->id(), 'enable_user_settings', TRUE);
    $this->userData->set('the_strata_theme', $account->id(), 'settings', $settings);
  }

  /**
   * Clears all the_strata_theme settings for the current user.
   *
   * @param \Drupal\Core\Session\AccountInterface|null $account
   *   The account object. Current user if NULL.
   */
  ...
  public function userOverrideEnabled(?AccountInterface $account = NULL) {
    ...
    if (!isset($overrides[$account->id()])) {
      $overrides[$account->id()] = $this->allowUserOverrides()
        && (bool) $this->userData->get('the_strata_theme', $account->id(), 'enable_user_settings');
    }
  ...
  public function getSettingsForm(?AccountInterface $account = NULL): array {
    ...
    $form['preset_accent_color'] = [
      ...
      '#options' => [
        'blue' => $this->t('TheStrataTheme Blue (Default)'),
      ...
      '#after_build' => [
        '_the_strata_theme_accent_radios',
      ],
    ];
    ...
    $form['preset_focus_color'] = [
      ...
      '#options' => [
        'gin' => $this->t('TheStrataTheme Focus color (Default)'),
        'green' => $this->t('Green'),
    ...
    // Toolbar setting.
    $is_navigation_active = _the_strata_theme_module_is_active('navigation');
    ...
    $form['classic_toolbar'] = [
      ...
      '#after_build' => [
        '_the_strata_theme_toolbar_radios',
      ],
    ];
    ...
    if (!$account) {
      foreach ($form as $key => $element) {
        $form[$key]['#after_build'][] = [
          TheStrataThemeAfterBuild::class, 'overriddenSettingByUser',
        ];
      }
    }
